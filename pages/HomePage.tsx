...
import { PROMPTS_BY_TOPIC, TOPICS, PRACTICE_TYPES, IMAGE_PROMPTS, SCENARIO_PROMPTS, LEVELS } from '../constants';
...
import { SessionLog, PracticeType, Prompt, Level } from '../types';
...
const HomePage: React.FC<HomePageProps> = ({ saveSessionLog }) => {
  ...
  const generateNewPrompt = useCallback((type: PracticeType, topic: string, level: Level) => {
    setCurrentPrompt(current => {
      let promptList: any[] = [];
      if (type === 'conversation') {
        promptList = PROMPTS_BY_TOPIC[topic]?.[level] || [];
      } else if (type === 'image') {
        promptList = IMAGE_PROMPTS.filter(p => p.level === level);
      } else if (type === 'scenario') {
        promptList = SCENARIO_PROMPTS.filter(p => p.level === level);
      }

      if (promptList.length === 0) {
        const fallbackPromptContent = `No ${level} prompts available for this selection. Please choose another level or topic.`;
        return { type, content: fallbackPromptContent, level };
      }

      if (promptList.length === 1) {
        const item = promptList[0];
        if (type === 'conversation') return { type, content: item, level };
        return { type, ...item };
      }
      ...
      // Fallback if a new prompt wasn't found
      if (!newPrompt) {
        const randomItem = promptList[Math.floor(Math.random() * promptList.length)];
        if (type === 'conversation') {
          newPrompt = { type, content: randomItem, level };
        } else {
          newPrompt = { type, ...randomItem };
        }
      }
  ...
  useEffect(() => {
    ...
    generateNewPrompt(practiceType, selectedTopic, selectedLevel);
  }, []);
  ...
  const handlePracticeTypeChange = (type: PracticeType) => {
    ...
    generateNewPrompt(type, selectedTopic, selectedLevel);
  };

  const handleLevelChange = (level: Level) => {
    setSelectedLevel(level);
    generateNewPrompt(practiceType, selectedTopic, level);
  };

  const handleTopicChange = (e: React.ChangeEvent<HTMLSelectElement>) => {
    const newTopic = e.target.value;
    setSelectedTopic(newTopic);
    generateNewPrompt(practiceType, newTopic, selectedLevel);
  };
      ...
      <div className="max-w-4xl mx-auto space-y-6 pb-16">
        <Card>
          <CardContent className="p-4 flex flex-col sm:flex-row items-start sm:items-center gap-4 flex-wrap">
            ...
            <div className="flex items-center gap-2 flex-wrap">
              <span className="font-medium text-sm text-muted-foreground">Level:</span>
              {LEVELS.map(level => (
                  <Button
                      key={level.id}
            ...
            <Button
                variant="outline"
                size="sm"
                onClick={() => generateNewPrompt(practiceType, selectedTopic, selectedLevel)}
                disabled={isSessionActive}
